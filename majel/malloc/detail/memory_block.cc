...
void MemoryBlock::initialize(MetadataCache& cache,
                             Type t,
                             size_t index,
                             size_t size,
                             void* left_buddy,
...
void MemoryBlock::split(MetadataCache& cache, size_t size) {
  ...
  cache.store(static_cast<MemoryBlock*>(right_partition),
              MemoryBlockMetadata(FREE_MEMORY,
                                  index(cache),
                                  remaining_size - overhead(),
                                  remaining_size,
                                  this,
...
size_t MemoryBlock::overhead() { return sizeof(MemoryBlockMetadata); }
}
}
}
