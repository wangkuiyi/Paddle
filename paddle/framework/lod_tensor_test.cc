...
class LoDTensorTester : public ::testing::Test {
 public:
  virtual void SetUp() override {
    ...
    // malloc memory
    lod_tensor_.mutable_data<float>(place_);
...
TEST_F(LoDTensorTester, SliceLevels) {
  ...
  for (size_t level = 0; level < 3UL; ++level) {
    ...
    ASSERT_EQ(new_lod_tensor.data<float>(),
              lod_tensor_.data<float>());
  }
  // slice 2 level
  for (size_t level = 0; level < 2UL; ++level) {
    ...
    ASSERT_EQ(new_lod_tensor.NumElements(0), lod_tensor_.NumElements(level));
    ASSERT_EQ(new_lod_tensor.NumElements(1), lod_tensor_.NumElements(level + 1));
    ASSERT_EQ(new_lod_tensor.data<float>(),
              lod_tensor_.data<float>());
  }
}
